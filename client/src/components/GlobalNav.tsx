            ...
            <div className="hidden md:flex items-center gap-6">
              {navLinks.map((link) => (
                <Link key={link.path} href={link.path}>
                  <span className={`font-mono text-xs tracking-widest uppercase transition-colors cursor-pointer ${location === link.path ? "text-white" : "text-zinc-500 hover:text-zinc-300"
                    }`}>
                    {link.name}
                  </span>
                </Link>
